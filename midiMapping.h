...
protected:
    ...
    {
        plugin->setValue(valueIndex, message->at(1) / 128.0f);
        return true;
    }
    ...
    bool handleUint8Position2(std::vector<unsigned char>* message)
    {
        if (msg[1] == message->at(1)) {
            plugin->setValue(valueIndex, message->at(2) / 128.0f);
            return true;
        }
        return false;
    }
    ...
    {
        plugin->setValue(valueIndex, ((message->at(2) << 7) + message->at(1)) / 16383.0f);
        return true;
    }
